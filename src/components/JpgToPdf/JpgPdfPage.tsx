...
interface JpgPdfPageProps {
  onBack: () => void;
}

const JpgPdfPage: React.FC<JpgPdfPageProps> = ({ onBack }) => {
  ...
  return (
    <div className="relative min-h-[calc(100vh-4rem)] overflow-hiddenbg-gradient-to-br from-indigo-50 via-purple-50 to-pink-100">
      {/* Dynamic Background */}
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="text-center mb-10 animate-fade-in-up">
          ...
          <p className="max-w-xl mx-auto text-base text-neutral-500 leading-relaxed font-medium">
            Transform your images into high-quality PDF documents in seconds.
            No file limits, no watermarks, just simple and secure conversion.
          </p>
        </div>
        ...
        <div className={`
             ...
             ${images.length > 0 ? 'lg:grid-cols-12' : 'max-w-3xl mx-auto'}
        `}>

          {/* Upload Section */}
          <div className={`
              transition-all duration-500
              ${images.length > 0 ? 'lg:col-span-5' : 'w-full'}
          `}>
              ...
              <div className="bg-white/50 rounded-[1.8rem] p-6 border border-white/50">
                ...
                {images.length === 0 && (
                  <div className="mt-6 grid grid-cols-3 gap-3 text-center">
                    {[
                      { label: 'Secure', desc: 'Client-side only', icon: '🔒', color: 'bg-emerald-50 text-emerald-600' },
                      { label: 'Fast', desc: 'Instant Process', icon: '⚡', color: 'bg-amber-50 text-amber-600' },
                      { label: 'Free', desc: 'No Hidden Fees', icon: '💎', color: 'bg-rose-50 text-rose-600' },
                    ].map((item, idx) => (
